...
public class Piece {
	...
	public Piece(TPoint[] points) {
		...
		for (int i = 0; i < points.length; i++) {
			...
			if (i == 0 || width < points[i].x) {
				width = points[i].x;
			}
			if (i == 0 || height < points[i].y) {
				height = points[i].y;
			}
		}
		max_x = width;
		...
		int check_x = 0;
		for (int i = 0; i < 4; i++) {
			if (i == 0 || check_x != points[i].x) {
				check_x = points[i].x;
				skirt[check_x] = points[i].y;
			} else if (skirt[check_x] > points[i].y) {
				skirt[check_x] = points[i].y;
			}
		}
	}
	...
	public Piece computeNextRotation() {
		TPoint[] rotate_point = new TPoint[4];
		int max = height;
		for (int i = 0; i < rotate_point.length; i++) {
			int temp_x = max - body[i].y - 1;
			int temp_y = body[i].x;
			rotate_point[i] = new TPoint(temp_x, temp_y);
		}
		Piece temp = new Piece(rotate_point);
